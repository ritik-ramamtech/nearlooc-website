...
export function FavoriteCard({ offer, onRemove, isRemoving }: FavoriteCardProps) {
  return (
    <div className="w-full">
      ...
      >
        {/* Image — same 24:19 ratio as OfferCard (240×190) */}
        <div className="relative aspect-[24/19] w-full overflow-hidden bg-gray-100">
          {offer.image_url ? (
          ...
          {/* Remove / heart button */}
          <button
            onClick={() => { if (!isRemoving) onRemove(offer.id); }}
            disabled={isRemoving}
            style={{
              position: "absolute",
              bottom: 10,
          ...
          >
            <Heart style={{ width: 14, height: 14 }} className="fill-red-500 text-red-500" />
          </button>
        </div>

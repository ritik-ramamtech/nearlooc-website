...
import {
  ArrowLeft, Plus, Tag, Power, Percent, Clock, Star, X,
} from "lucide-react";
      ...
      <div className="p-6 max-w-3xl mx-auto space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-[#1a5c2a] shadow-sm p-6 space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2">
                ...
                />
                <button
                  onClick={() => { addTag(features, setFeatures, featureInput); setFeatureInput(""); }}
                ...
                <div className="flex flex-wrap gap-2">
                  {features.map((f) => (
                    <span key={f} className="flex items-center gap-1 text-xs px-2.5 py-1 bg-[#e8f5e9] text-[#1a5c2a] rounded-full font-medium">
                      {f}
                      <button onClick={() => setFeatures(features.filter((x) => x !== f))}><X className="h-2.5 w-2.5" /></button>
                    </span>
                  ))}
                </div>
              )}
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2">
                ...
                />
                <button
                  onClick={() => { addTag(terms, setTerms, termInput); setTermInput(""); }}
                  className="px-3 py-2.5 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 transition-colors"
                >
                  <Plus className="h-4 w-4" />
                </button>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {terms.map((t) => (
                    <span key={t} className="flex items-center gap-1 text-xs px-2.5 py-1 bg-gray-100 text-gray-600 rounded-full font-medium">
                      {t}
                      <button onClick={() => setTerms(terms.filter((x) => x !== t))}><X className="h-2.5 w-2.5" /></button>
                    </span>
                  ))}
                </div>
              )}
                      ...
                      <label key={loc.id} className="flex items-center gap-2.5 cursor-pointer p-2 rounded-lg hover:bg-[#f0f7f0] transition-colors">
                        <div
                          ...
                          className={`h-4 w-4 rounded border-2 flex items-center justify-center transition-colors shrink-0 ${
                            selectedLocations.includes(loc.id) ? "bg-[#1a5c2a] border-[#1a5c2a]" : "border-gray-300 bg-white"
                          }`}
                        >
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                ...
                disabled={creating}
                className="flex-1 px-5 py-2.5 bg-[#1a5c2a] hover:bg-[#14471f] text-white rounded-lg text-sm font-semibold transition-colors disabled:opacity-60"
              >
        ...
        <Section title="Active Offers" count={productOffers.length} loading={loadingActive}>
          {productOffers.length === 0 ? (
            <EmptyOffers label="No active offers" desc="Create an offer above to start promoting this product." />
          ) : (
            productOffers.map((o) => (
...
function OfferCard({ offer, onDeactivate, showDeactivate }: {
  offer: import("@/types/merchant").MerchantSale;
  onDeactivate: () => void;
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex items-center justify-between gap-4">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-400 line-through">
              ₹{offer.product?.base_price?.toLocaleString("en-IN")}
            </span>
            <span className="text-xs font-semibold text-[#1a5c2a]">{offer.discount_percentage}% off</span>
          </div>
          <div className="flex items-center gap-2 mt-0.5">
            {offer.badge && (
              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-orange-100 text-orange-600">{offer.badge}</span>
            )}
            {offer.promo_end_at && (
              <span className="text-[10px] text-gray-400 flex items-center gap-0.5">
                <Clock className="h-3 w-3" />
                Ends {new Date(offer.promo_end_at).toLocaleDateString("en-IN")}
              </span>
            )}
            {offer.rating > 0 && (
              <span className="text-[10px] text-gray-400 flex items-center gap-0.5">
      ...
      {showDeactivate && offer.is_active && (
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-red-500 border border-red-200 rounded-lg hover:bg-red-50 transition-colors shrink-0"
        >
      ...
      {!offer.is_active && (
        <span className="text-[10px] font-semibold px-2 py-1 rounded-full bg-gray-100 text-gray-400">Inactive</span>
      )}
    </div>
  );
}

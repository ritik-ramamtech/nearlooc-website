...
import {
  useCreateProduct,
  useUpdateProduct,
} from "@/features/merchant/products/hooks";
...
import { MarkdownTextarea } from "@/components/form/markdown-textarea";
...
export default function ProductForm(params: ProductFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    if(!imageFile) {
      setError("At least one image of product is required.");
      return;
    }
    setError(null);
  ...
  useEffect(() => {
    ...
    setSelectedLocations(initialData.locations?.map((loc) => loc.id) ?? []);
  }, [initialData]);
